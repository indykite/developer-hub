---
name: erp-invoices
description: Query the ERP invoices backend (erp_* tools) - account fee invoices whose rows are pre-filtered per caller by AuthZEN search/resource over the knowledge graph. Use for invoices, billing, account fees due, payment status.
tags:
  - erp
  - invoices
  - billing
...
- Different logins legitimately see different rows (staff see the customers
  their department serves). A small result is not an error.
