...
Canbank is a retail bank. Canbank is adding AI agents to help their employees
support their customers. In this demo there is a chatbot and two agents deployed: an orchestrator and a retriever. Each agent is protected by the IndyKite Agent Gateway.
...
1. IndyKite chatbot
2. terminal with docker ps -q --filter "name=iag-token-exchange-retriever-1" | xargs docker logs -f or
   docker logs -f $(docker ps -q --filter "name=iag-token-exchange-retriever-1")
...
Canbank is adding AI agents to help their employees provide better support for their customers.
