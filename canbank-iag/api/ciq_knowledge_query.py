    ...
    "upsert_nodes": [
        {
            ...
            "properties": [
                {"type": "description", "value": "$decision_description"},
            ],
        },
    ],
    "upsert_relationships": [
        {"name": "r_applies", "source": "decision", "target": "document", "type": "APPLIES"},
        {"name": "r_made", "source": "subject", "target": "decision", "type": "MADE"},
        {"name": "r_closed", "source": "ticket", "target": "decision", "type": "CLOSED_BY"},
    ],
    "nodes": [
        "decision.external_id",
        "decision.property.description",
...
_QUERY_DEFS = [
    ...
    },
    {
        ...
        "description": (
            "Get Stock Quote retrieves the user, their department and a stock quote for the requested ticker symbol."
    ...
    },
    {
        ...
        "description": (
            "Store Decision records a new decision in the knowledge graph: it creates a "
            "Decision node and links it to the policy document it applies (APPLIES), the "
            "support ticket it closes (CLOSED_BY) and the authenticated user who made it "
            "(MADE), all in one atomic operation. Use this function when a decision has "
            "been reached and must be persisted, such as: 'Record that I approved this "
            "refund as an exception to the refund policy.' Requires the user's bearer "
            "token. Call tool 'ciq_execute' with the following arguments: \"id\": "
            '"store-decision" and input_params decision_external_id, '
            "decision_description, document_external_id, ticket_external_id. Example: "
            '{ "id": "store-decision", "input_params": { "decision_external_id": '
            '"decision_002", "decision_description": "Refund approved as an exception", '
            '"document_external_id": "refund_policy", "ticket_external_id": "ticket_001" } }.'
        ),
        "query": QUERY_10,
    },
]
