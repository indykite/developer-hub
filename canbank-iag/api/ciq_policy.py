...
POLICY_4 = {
    ...
    "allowed_reads": {
        "nodes": ["subject.*", "document.*", "taxonomy.*"],
        "relationships": [],
        "aggregate_values": [],
    },
}
...
# Policy 10: Store Decision — a user records a new Decision node, linked to the document it
# applies, the ticket it closes, and the user who made it (a CIQ write / upsert policy).
POLICY_10 = {
    "meta": {"policy_version": "1.0-ciq"},
    "subject": {"type": "User"},
    "condition": {
        "cypher": (
            "MATCH (subject:User {external_id: $token.sub}) "
            "MATCH (document:Document {external_id: $document_external_id}) "
            "MATCH (ticket:Ticket {external_id: $ticket_external_id})"
        ),
        "filter": [],
    },
    "allowed_upserts": {
        "nodes": {"node_types": ["Decision"]},
        "relationships": {
            "relationship_types": [
                {
                    "type": "APPLIES",
                    "source_node_label": "Decision",
...
_POLICY_DEFS = [
    ...
    },
    {
        ...
        "description": (
            "Create a new Decision node and wire it to the document it applies (APPLIES), "
            "the ticket it closes (CLOSED_BY) and the authenticated user who made it (MADE) "
            "in one atomic CIQ upsert."
        ),
        "policy": POLICY_10,
        "tags": ["canbank", "decisions", "write"],
    },
]
