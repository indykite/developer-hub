...
info = Info(title="Instant Stack API", version="1.0.0")
security_schemes = {
...
@app.get("/")
def index():
    """Render the app landing page, cataloguing the active dataset's manifest."""
    return render_template(
        "index.html",
        dataset=_dataset.DATASET,
        datasets=_dataset.available_datasets(),
        manifest_display_name=_dataset.DISPLAY_NAME,
        manifest_description=_dataset.DESCRIPTION,
        ciq_queries=_dataset.CIQ_QUERIES,
        ciq_policies={p["slot"]: p for p in _dataset.CIQ_POLICIES},
        resolvers=_dataset.RESOLVERS,
        kbac_policies=_dataset.KBAC_POLICIES,
    )
